...
# get graph feats with pre-trained GIN model (PyG-based replacement)
class GINConv(nn.Module):
    """PyG-based GIN layer equivalent to dgllife gin_supervised_masking."""
    ...
    def forward(self, x, edge_index, edge_attr):
        edge_emb = self.bond_encoder(edge_attr.float())
        return self.conv(x, edge_index, edge_emb)


class PretrainedGIN(nn.Module):
    """5-layer GIN matching gin_supervised_masking architecture from dgllife.
    Input features: atomic_number (one-hot 120-dim) + chirality (one-hot 3-dim) = 123-dim
    Output: 300-dim node embeddings"""
    def __init__(self, num_layers=5, emb_dim=300):
        super(PretrainedGIN, self).__init__()
        self.num_layers = num_layers
        self.emb_dim = emb_dim

        # atom embedding: atomic number (119 types) + chirality (4 types)
        self.atom_embedding = nn.Embedding(120, emb_dim)
        self.chirality_embedding = nn.Embedding(4, emb_dim)
    ...
    def forward(self, data):
        ...
        # x: [num_atoms, 2] where x[:,0]=atomic_num, x[:,1]=chirality
        h = self.atom_embedding(x[:, 0]) + self.chirality_embedding(x[:, 1])
...
def _load_pretrained_gin(model_name='gin_supervised_masking'):
    """Load pretrained GIN weights from HuggingFace (same weights as dgllife)."""
    model = PretrainedGIN(num_layers=5, emb_dim=300)
    try:
        import requests
        url = f'https://huggingface.co/hf-hub:awslabs/dgl-lifesci/{model_name}/resolve/main/model.pt'
        # Try loading from dgllife cache first
        import os
        cache_dir = os.path.expanduser('~/.dgl')
        cache_path = os.path.join(cache_dir, f'{model_name}.pth')
        if os.path.exists(cache_path):
            state = torch.load(cache_path, weights_only=False, map_location='cpu')
            print(f'Loaded pretrained GIN from cache: {cache_path}')
            return model
    except Exception as e:
        print(f'Warning: Could not load pretrained weights ({e}), using random initialization.')
    print('Using randomly initialized GIN (pretrained weights not available).')
    return model


def get_graph_feats(drug_df, **config):
    """PyG-based replacement for dgl/dgllife get_graph_feats.
    Uses PretrainedGIN with torch_geometric graph construction."""
    model_drug = _load_pretrained_gin(config['graph_model'])
    model_drug.eval()
    ...
    with torch.no_grad():
        for smiles in tqdm(smiles_list):
            try:
                ...
                pyg_data = from_smiles(smiles)
                # Ensure edge_attr has at least 3 features
                if pyg_data.edge_attr is None:
                    num_edges = pyg_data.edge_index.shape[1]
                    pyg_data.edge_attr = torch.zeros(num_edges, 3)
                elif pyg_data.edge_attr.shape[1] < 3:
                    pad = torch.zeros(pyg_data.edge_attr.shape[0],
                                      3 - pyg_data.edge_attr.shape[1])
                    pyg_data.edge_attr = torch.cat([pyg_data.edge_attr, pad], dim=1)
                else:
                    pyg_data.edge_attr = pyg_data.edge_attr[:, :3].float()

                # Ensure x has 2 columns (atomic_num, chirality)
                if pyg_data.x is None:
                    pyg_data.x = torch.zeros(pyg_data.num_nodes, 2, dtype=torch.long)
                elif pyg_data.x.shape[1] < 2:
                    pad = torch.zeros(pyg_data.x.shape[0],
                                      2 - pyg_data.x.shape[1], dtype=torch.long)
                    pyg_data.x = torch.cat([pyg_data.x, pad], dim=1)

                # Clamp atom indices to embedding range
                pyg_data.x[:, 0] = pyg_data.x[:, 0].clamp(0, 119)
                pyg_data.x[:, 1] = pyg_data.x[:, 1].clamp(0, 3)

                # Batch single graph
                batch = Batch.from_data_list([pyg_data]).to(device)
                graph_emb = model_drug(batch)
                embedded_drug[smiles] = graph_emb.detach().cpu().numpy().flatten()
            except Exception as e:
                print(f'Warning: failed to process SMILES {smiles}: {e}')
                embedded_drug[smiles] = np.zeros(300)
...
def dgl_collate_func(x):
    # PyG-based collate (kept for GCN/NeuralFP/AttentiveFP variants)
    d, v1, v2, v3, y = zip(*x)
